...
tikz_save("plots/narrative_speech.tex")
plt.show()
...
for node in NN():
    ...
    if otype == "verse":
        ...
        DSU = (F.etcbc4_ft_txt.v(node).count("Q") > 0)    # If the text_type contains a Q, we consider the clause as Direct Speech
    elif F.etcbc4_db_otype.v(node) == "word" and DSU == False:
        potential_ref = False
        potential_suffix = False
        if not (F.etcbc4_ft_ps.v(node) in ("unknown", "NA") and F.etcbc4_ft_nu.v(node) in ("unknown","NA") and F.etcbc4_ft_gn.v(node) in ("unknown","NA")):
            ...
            if not phrase in nar_phrases and DSU == False:
                nar_phrases.append(phrase)
            if not phrase in speech_phrases and DSU == True:
                speech_prhases.append(phrase) 
...
\begin{python}
# NOTE: this function and the next one still need to be adapted from working with one data dict to working with two (nar_data and speech_data)
nonverbs = []
verbs = []

for phrase in ref_phrases:
    if F.etcbc4_ft_function.v(phrase) in ('Subj','Pred','PreC','PreO'):
        data[phrase] = {}
        ...
        if len(phrase_cons) > 1:
            if F.etcbc4_ft_sp.v(phrase_nodes[0]) == "nmpr" or F.etcbc4_ft_ls.v(phrase_nodes[1]) == "nmpr":
            ...
            if F.etcbc4_ft_sp.v(phrase_nodes[-1]) == "verb":
                if F.etcbc4_ft_vt.v(phrase_nodes[-1]) == "ptca":
                    ...
                    verbs.append((L.u('clause',phrase),phrase))
...
# Check fails, for there are compount subjects. These are multiple referents in the same clause

check = {}
for key in data:
    if L.u('clause',key) in check:
#        print("Oops!", key)
        check[L.u('clause',key)].append((key,data[key]["destype"]))
    else:
        check[L.u('clause',key)] = []
        check[L.u('clause',key)].append((key,data[key]["destype"]))
...
t2 = df['destype'].value_counts()
t3 = Series(t2,index=["verb","ptca","suffix","personal_pronoun","demonstr_pronoun","adjective","ptcp","noun","phrase","name","name_phrase"])
t3.plot(kind="bar")
plt.show()
