...
for node in NN():
    ...
    if otype == "book" and F.etcbc4_sft_book.v(node) != "Genesis":
        break
    elif otype == "chapter" and int(F.etcbc4_sft_chapter.v(node)) >= 12:
        break
    elif otype == "verse":
        ...
        if F.etcbc4_ft_rela.v(node) == "rec":
            ...
            for x in C.mother.v(node):
                REG = x
                same = False
                for y in give_me_your_parents_up_to(x):
                    if F.etcbc4_db_otype.v(y) == "phrase" and F.etcbc4_ft_number.v(y) == phrase:
                        same = True
            if same == False:
                print(verse, node,same, "REG: ",F.etcbc4_ft_g_cons.v(REG)," rectum: ", get_cons_of_word_list(give_me_your_words(node)))
