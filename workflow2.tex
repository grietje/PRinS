...
#(1=Part_suffix, 2=Part_word, 3=Part_(sub)phrase, 4=Part_clause, 5=Part_sentence): ")
...
def ask_potential_ref(place, candidate, node_type, sub):
    if node_type == "word+": 
        data[candidate]= {}
        data[candidate]["Des"] = get_cons_of_word_list(give_me_your_words(candidate))
        data[candidate]["a"] = place
        data[candidate]["Node"] = candidate
        nodetype = F.etcbc4_db_otype.v(candidate)
        if nodetype == "word":
        ...
        recognize = recognize(get_cons_of_word_list(give_me_your_words(candidate)))
        if not recognize == []:
        ...
        candidate = str(candidate)+"s"
        data[candidate] = {}
        data[candidate]["Des"] = F.etcbc4_ft_prs.v(pure_candidate)
        data[candidate]["Nodetype"] = "1"
        data[candidate]["a"] = place
        data[candidate]["Node"] = pure_candidate
...
def interactive_single(data[candidate]):
    ...
    print("clause:", get_cons_of_word_list(give_me_your_words(L.u('clause',data[candidate]["Node"]))))   print("suffix:", data[candidate]["Des"])
...
def interactive_clan(data,clan_list):
    print("Clause:", get_cons_of_word_list(give_me_your_words(L.u('clause',clan_list[-1]))))
    ...
    for candidate in clan_list:
        print("{0!s: <9} {2!s: <15} {3!s: <40} {4!s: <30}".format(data[candidate]["Node"], data[candidate]["Sub"], get_cons_of_word_list(give_me_your_words(candidate))
        print("{0!s: <1} {2!s: <1} {3!s: <10} {4!s: <1} {5!s: <1} {6!s: <1}".format(data[candidate]["Nodetype"], data[candidate]["Destype"], data[candidate]["Name"], data[candidate]["Anim"], data[candidate]["Human"], data[candidate]["Col"]))
    for candidate in clan_list:
        for x in ("Nodetype","Destype","Name","Anim","Human","Col"):
            if data[candidate][x] == None:
                data[candidate][x] = input(x + "of node" + candidate + ": ")
...
def major_flow(place, node, node_type):
    ...
    for candidate in clan:
        clan_list.append(candidate)
        data[candidate] = ask_potential_ref(place, candidate, "word+", clan[candidate])
        if not F.etcbc4_ft_prs.v(candidate) in ("absent","n/a",None):
            clan_list.append(str(candidate)+"s")
            data[candidate] = ask_potential_ref(place, candidate, "suffix", clan[candidate])
    interactive_clan(data,clan_list)
\end{python}
...
#        if check_node != "":     # Just a hack when temporarily outcommenting lines below
        ...
        if result == (0,) and potential_ref == False and potential_suffix == True: # Not yet saved, not to be dealt with in major flow, but suffix needs to be parsed
            data[node] == ask_potential_ref(place, node, "single_suffix")
