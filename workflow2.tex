...
#        ask_potential_ref(candidate, clan[candidate], place, Clause, ClANr, Phrase, PhANr, SPhNr, WNr):
#        ask_potential_ref(node, node_type, place, Clause, ClANr, Phrase, PhANr, SPhNr, WNr)
#        tag = place.rjust(15,' ') + "_" + str(ClANr).rjust(9,'0') + "_" + str(PhANr).rjust(9,'0') + "_" + str(WNr).rjust(9,'0') + "_" + F.etcbc4_db_otype.v(candidate) # Warning: otype not yet encoded
#        if not recognize(clan[tag][0]) == []:
#            data[tag] = {}
#            data[tag]["Name"] = 
#            data[tag]["Anim"] = 
#            data[tag]["Human"] = 
def ask_potential_ref(candidate, node_type, data[candidate], place, Clause, ClANr, Phrase, PhANr, SPhNr, WNr):
        Data[candidate]= {}
        print("-"*20)
        print(place)
...
def major_flow(place, node, node_type, Clause, ClANr, Phrase, PhANr, SPhNr, WNr):
    ...
    clan = {}
    data = {}
    clan[node] = [get_cons_of_word_list(give_me_your_words(node))]
    for candidate in give_me_your_daughters(node):
        clan[candidate]= [get_cons_of_word_list(give_me_your_words(candidate))]
    for candidate in clan:
        ask_potential_ref(candidate, "clanmember", clan[candidate], place, Clause, ClANr, Phrase, PhANr, SPhNr, WNr)
        if not F.etcbc4_ft_prs.v(candidate) in ("absent","n/a",None):
            ask_potential_ref(candidate, "suffix", clan[candidate], place, Clause, ClANr, Phrase, PhANr, SPhNr, WNr)
\end{python}
...
#        if check_node != "":     # Just a hack when temporarily outcommenting lines below
        ...
        if result == (0,) and potential_ref == True: # Not yet saved, to be dealt with in major flow (may have suffixes as well, these will be parsed within the major flow)
            major_flow(place, node, "word", Clause, ClANr, Phrase, PhANr, SPhNr, WNr)
        if result == (0,) and potential_ref == False and potential_suffix == True: # Not yet saved, not to be dealt with in major flow, but suffix needs to be parsed
            ask_potential_ref(node, "single_suffix", [], place, Clause, ClANr, Phrase, PhANr, SPhNr, WNr)
