...
\node [outline=blue, text width=7.5em] (clan) [below = of flow] {Interactive\_clan *Add\_data};
...
#        print("no problem so far")
        if F.etcbc4_ft_function.v(L.u('phrase',cand)) in ("ExsS","IntS","ModS","NCoS","PrcS","PreS"):
            ...
            current["Role"] = "Prep" # Will be checked interactively
#        print("No problem so far 2")
    return(current)
...
def interactive_single(data,candidate):
    ...
    if current["Col"] in ("2","3","4"):
        ...
        current["ColPart"] = input("Contextually relevant collective of which Name is part: ") or None
    current["Role"] = input("Role in sentence? Default is " + current["Role"]) or current["Role"]
...
for node in NN():
    ...
    if otype == "book" and F.etcbc4_sft_book.v(node) != "Genesis":
        break
#    elif otype == "chapter":
#        if int(F.etcbc4_sft_chapter.v(node)) < 10:    # Prevent re-doing already completed chapters
#            completed = True
#        elif int(F.etcbc4_sft_chapter.v(node)) > 3:    # Set a limit on the amount of data dealt with this time
#            break
#        else:
