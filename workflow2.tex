...
#        cand = candidate[:-1]
    ...
    if current["Tag"] in list(cursor.execute("select Tag from pt")): # Double-check whether this works -- it doesn't
...
todo = list(cursor.execute("select a,Node,Nodetype,Tag from pt"))

for (a,Node,Nodetype,Tag) in todo:
    Node = int(Node)
    ...
    if Tag2 in list(cursor.execute("select Tag from pt")[0]):
        Tag2 = Tag2+"0"
        if Tag2 in list(cursor.execute("select Tag from pt")[0]):
            Tag2 = Tag2[:-1]+"1"
            if Tag2 in list(cursor.execute("select Tag from pt")[0]):
    ...
    cursor.execute("update pt set Tag=:Tag2 where Tag=:Tag"
