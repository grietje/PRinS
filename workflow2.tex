...
\begin{python}
# Add Role 
todo = list(c.execute("select Node,Tag,a,Nodetype,Sub,Des from pt where Role is NULL"))

for (Node,Tag,a,Nodetype,Sub,Des) in todo:
    if "Rec" in str(Sub):
        Role = "Pos ?"
    else:
        Role = F.etcbc4_ft_function.v(L.u('phrase',int(Node))) 
        if Role == None:
            Role = F.etcbc4_ft_function.v(int(Node))
    print(Node, a, Nodetype, Sub, Des, Role)
    c.execute("update pt set Role=:Role where Node=:Node and Nodetype=:Nodetype", {"Role":Role,"Node":Node,"Nodetype":Nodetype})
...
freqNonSubj = {}
for Name,_ in topnames:
    freqSubj[Name] = list(c.execute("select Destype,Count(*) from pt where Name = :Name and Role in ('Subj','Pred','PreC','PreO') group by Destype order by Destype", {"Name":Name}))
    freqNonSubj[Name] = list(c.execute("select Destype,Count(*) from pt where Name = :Name and Role not in ('Subj','Pred','PreC','PreO') group by Destype order by Destype", {"Name":Name}))
...
dfSubj2
dfSubj2.plot(kind="bar",stacked=True,title = "Subject or predicate")
...
dfNonSubj2
dfNonSubj2.plot(kind="bar",stacked=True,title = "Non-subject")
