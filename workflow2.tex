...
def ask_potential_ref(place, candidate, node_type, sub):
    if node_type == "word+": 
        ...
        data[candidate] = {}
        data[candidate]["Des"] = F.etcbc4_ft_prs.v(candidate[:-1])
        data[candidate]["Nodetype"] = "1"
        data[candidate]["a"] = place
        data[candidate]["Node"] = candidate[:-1]
...
def interactive_clan(data,clan_list):
    ...
    for candidate in clan_list:
        ...
        print("{0!s: <2} {2!s: <2} {3!s: <10} {4!s: <2} {5!s: <2} {6!s: <2}".format(data[candidate]["Nodetype"], data[candidate]["Destype"], data[candidate]["Name"], data[candidate]["Anim"], data[candidate]["Human"], data[candidate]["Col"]))
    for candidate in clan_list:
...
#        if check_node != "":     # Just a hack when temporarily outcommenting lines below
        c.execute("SELECT EXISTS (SELECT * FROM pt WHERE Node = " + check_node + ");") # Check whether reference is not yet saved in database
        result = c.fetchone()
        potential_ref = False
        potential_suffix = False
