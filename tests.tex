...
#nar3.index.name = "Destypes after \hebr{>T} in Narrative"
...
#speech3.index.name = "Destypes after \hebr{>T} in Direct Speech"
...
\begin{python}
# Make histogram for destypes of Rec and Attr subparticipants (only from gcdata.pt)
todo = list(cursor.execute("select Node,Destype,Sub from pt where not Sub = 'Sub1' and not Sub like '%Sub1' and not Sub = 'Super' and not Sub like '%Super' and not Sub = 'Super1' and not Sub like '%0%' and not Sub = '0' and not Sub = 'None'"))
# Excluded references ending with "Sub1" because these are only an artificially constructed part of a "Super" participant, not a real sub-participant. The Super participants themselves are not sub-participants but extensive normal participants. References with a 0 are mostly part of >CR clauses, which are complex cases with almost "normal" slots (not necessarily Rec/Attr/Prep). I left them out for now.
...
for (Node,Destype,Sub) in todo:
    if str(Sub).strip().endswith("?"):
        Sub = str(Sub).replace("?","")
    if str(Sub).strip().endswith("Super1"):
        Sub = str(Sub).replace("Super1","")
#        print(Sub)
    if not str(Sub).strip().endswith('Sub1') and not str(Sub).strip() in (" ","None","X",None):
        if str(Sub).strip().endswith("Rec") or str(Sub).strip().endswith("RecPre"):
            Rec += 1
            if Destype in Destypes["Rec"]:
                Destypes["Rec"][Destype] += 1
            else:
                Destypes["Rec"][Destype] = 1
        elif str(Sub).strip().endswith("Attr") or str(Sub).strip().endswith("AttrPre"):
            Attr += 1
            if Destype in Destypes["Attr"]:
                Destypes["Attr"][Destype] += 1
            else:
                Destypes["Attr"][Destype] = 1
        elif str(Sub).strip().endswith("Prep"):
            Prep += 1
            if Destype in Destypes["Prep"]:
                Destypes["Prep"][Destype] += 1
            else:
                Destypes["Prep"][Destype] = 1
        else:
            print(".",str(Sub),".")
...
\end{python}
\begin{python}
# Try to test Runge's S1 references
S1test = list(cursor.execute("select Node,Destype,Role,Name,Nr,ClANr,PhANr from pt where not Nr is NULL and not Nr = '' order by Name,Nr"))
Destypes_S = {}
Destypes_N = {}
Name_prev = ""
Role_prev = ""
...
for (Node,Destype,Role,Name,Nr,ClANr,PhANr) in S1test:
    if Role == "Pred": # NOTE: still need to exclude verbs following a more extensive reference? Or is this done already by "(ClANr_now-ClANr_Prev) == 1"?
        Role = "Subj"
    Name_now = Name
    Role_now = Role
#    Nr_now = Nr
    ClANr_now = ClANr
    PhANr_now = PhANr
    if Name_prev == Name_now and Role_now == Role_prev and (ClANr_now-ClANr_prev) in (0,1):
    #and not (PhANr_now == PhANr_prev):
#    and (Nr_now-Nr_prev) == 1 
        if (ClANr_now-ClANr_prev) == 0 and PhANr_now == PhANr_prev:
            same_clause.append(Node)
        if Role_now == "Subj":
            if Destype in Destypes_S:
                Destypes_S[Destype].append(Role_now)
            else:
                Destypes_S[Destype] = [Role_now]
        else:
            if Destype in Destypes_N:
                Destypes_N[Destype].append(Role_now)
            else:
                Destypes_N[Destype] = [Role_now]
            if Role in Roles:
                Roles[Role] += 1
            else:
                Roles[Role] = 1
...
print("Same role S:", Destypes_S, "Same role N:", Destypes_N)


\end{python}

\begin{python}
# Find out what the frequency distribution is for the functions of suffixes
nar_suffix = {}
speech_suffix = {}
function = None
nar_other = {}
...
#        break
    if otype == "clause":
        ...
        if not F.etcbc4_ft_prs.v(node) in ("absent","n/a"):
            potential_suffix = True
            if F.etcbc4_ft_function.v(L.u('phrase',node)) in ('PreS','ExsS','IntS','ModS','NCoS','PrcS'): # Subject suffix
                function = "subject on verb"
            elif F.etcbc4_ft_function.v(L.u('phrase',node)) in ('PreO','PtcO'): # Object suffix
                function = "object on verb"
            elif F.etcbc4_ft_sp.v(node) == "prep":
                function = "prep"
                if F.etcbc4_ft_function.v(L.u('phrase',node)) in prep:
                    prep[F.etcbc4_ft_function.v(L.u('phrase',node))] += 1
                else:
                    prep[F.etcbc4_ft_function.v(L.u('phrase',node))] = 1
            elif F.etcbc4_ft_sp.v(node) == "subs":
                function = "noun"
            elif F.etcbc4_ft_sp.v(node) == "verb" and F.etcbc4_ft_vt.v(node) == "infc":
                function = "infc"
                infc.append((node,F.etcbc4_sft_label.v(L.u('verse',node)),get_cons_of_word_list(give_me_your_words(node))))
            elif F.etcbc4_ft_sp.v(node) == "verb" and F.etcbc4_ft_vt.v(node) == "ptca":
                function = "ptca"
                ptca.append((node,F.etcbc4_sft_label.v(L.u('verse',node)),get_cons_of_word_list(give_me_your_words(node))))
            elif F.etcbc4_ft_sp.v(node) == "verb" and F.etcbc4_ft_vt.v(node) == "ptcp":
                function = "ptcp"
                ptcp.append((node,F.etcbc4_sft_label.v(L.u('verse',node)),get_cons_of_word_list(give_me_your_words(node))))
            elif F.etcbc4_ft_sp.v(node) == "adjv":
                function = "adjv"
                adjv.append((node,F.etcbc4_sft_label.v(L.u('verse',node)),get_cons_of_word_list(give_me_your_words(node))))
            else:
                function = "other"
                other = F.etcbc4_ft_function.v(L.u('phrase',node)) + " " + F.etcbc4_ft_sp.v(node) + " " + F.etcbc4_ft_vt.v(node)
            if DSU == False:
                if function in nar_suffix:
                    nar_suffix[function] += 1
                else:
                    nar_suffix[function] = 1
                if not other == None:
                    if other in nar_other:
                        nar_other[other].append(node)
                    else:
                        nar_other[other] = [node]
            elif DSU == True:
                if function in speech_suffix:
                    speech_suffix[function] += 1
                else:
                    speech_suffix[function] = 1
                if not other == None:
                    if other in speech_other:
                        speech_other[other].append(node)
                    else:
                        speech_other[other] = [node]

df = DataFrame.from_dict(nar_suffix,orient='index')
df1 = df.reindex(['subject on verb','infc','object on verb','ptca','noun','ptcp','adjv','prep','other'])
df2 = DataFrame.from_dict(speech_suffix,orient='index')
df3 = df2.reindex(['subject on verb','infc','object on verb','ptca','noun','ptcp','adjv','prep','other'])
...
tex += "\\begin{axis}[\n"
# Set up axis options
tex += "title = Functions of suffixes in narrative,\n"
tex += "xmin=-0.5, "
tex += "xmax=" + str(len(df1)-1) + ".5,\n"
tex += "ymin=0, "
tex += "ymax=" + str(nicebound(df1[0].max())) + ",\n"
tex += "xtick={" + ",".join([str(n) for n in range(len(df1))]) + "},\n"
tex += "xticklabels={" + ",".join(df1.index).replace("_","\_") + "},\n"
tex += "xticklabel style = {\\xticklabel},\n"
tex += "]\n"
# Draw data bars
...
tex += "\\begin{axis}[\n"
# Set up axis options
tex += "title = Functions of suffixes in direct speech,\n"
tex += "xmin=-0.5, "
tex += "xmax=" + str(len(df3)-1) + ".5,\n"
tex += "ymin=0, "
tex += "ymax=" + str(nicebound(df3[0].max())) + ",\n"
tex += "xtick={" + ",".join([str(n) for n in range(len(df3))]) + "},\n"
tex += "xticklabels={" + ",".join(df3.index).replace("_","\_") + "},\n"
tex += "xticklabel style = {\\xticklabel},\n"
tex += "]\n"
# Draw data bars
for i in range(len(df3)):
    if not str(df3[0][i]) in ("NaN","nan"):
        tex += "\\draw[draw=\\bardrawcolor,fill=\\barcolor] (axis cs:" + str(i - 0.25)
        tex += ",0) rectangle (axis cs:" + str(i + 0.25)
        tex += "," + str(df3[0][i]) + ");\n"
...
\begin{python}
# I am not content with the encoding of cardinal numbers. Try to add some consistency
data = {}
cardinals = list(cursor.execute("select ID,Node,a,Des,Destype,Nodetype,Sub,Name,Tag from pt where Lexset = 'card'"))
all_tags = list(cursor.execute("select Tag from pt order by Tag"))
for (ID,Node,a,Des,Destype,Nodetype,Sub,Name,Tag) in cardinals:
    if not Sub == None and not Sub.endswith("AttrPre") and not Sub.endswith("Attr"):
        index = [y[0] for y in all_tags].index(Tag)
        context = range(index-2,index+3)
        for x in context:
            data[x] = list(cursor.execute("select ID,Node,a,Des,Destype,Nodetype,Sub,Name,Tag from pt where Tag=:Tag2",{"Tag2":all_tags[x][0], }))
            print(x,data[x][0][0],data[x][0][1].rjust(10,"."),data[x][0][2],data[x][0][3].rjust(15,"."),data[x][0][4],data[x][0][5],data[x][0][6],data[x][0][7])
        newsub = input("Sub for the cardinal: ")
        cursor.execute("update pt set Sub=:newsub where Tag=:Tag",{"newsub":newsub,"Tag":Tag})

\end{python}

\begin{python}
# Fix that phrases have their main word as Node in gcdata.pt

todo = list(cursor.execute("select ID,Node,Nodetype,Destype,Tag from pt where Destype = 2 or Destype = 4"))
todo2 = []
for (Node,Nodetype,Destype,Tag) in todo:
    if Nodetype == '4':
        todo2.append((Node,Nodetype,Destype,Tag))
        newNode = L.u('subphrase',int(Node))
        if newNode == None:
            newNode = L.u('phrase',int(Node))
        print(F.etcbc4_db_otype.v(int(Node)),Tag,Nodetype,newNode,F.etcbc4_db_otype.v(newNode))
        cursor.execute("update pt set Node=:newNode where Tag=:Tag",{"newNode":newNode,"Tag":Tag})
