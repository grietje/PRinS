...
\begin{python}
#{{{ Find fitting info about types and functions # Does not yet work
...
#        cand = candidate[:-1]
    ...
    if current["Nodetype"] in ("4","5"): # Larger than phrase, so no PhANr
        current["PhANr"] = 00
    else:
        current["PhANr"] = L.u('phrase',current["Node"])
    if current["Nodetype"] in ("3","4","5"): # Larger than subphrase, so no SPhNr
        current["SPhNr"] = 00
    else:
...
\begin{python}
# New try to feed participants back to DSU
DSU_todo = list(cursor.execute("select DSUtag from gcdsu where Part is NULL or Part = '' or Part = '[]'"))

for DSUtag in DSU_todo:
    parts = []
    data = []
    references = list(cursor.execute("select Tag,Name,Referents,ColPart from pt where DSU=:DSUtag", {"DSUtag":DSUtag, }))
...
#    ColPart = list(cursor.execute("select ColPart from pt where Tag=:Tag", {"Tag":Tag, }))[0][0]
    if ColPart != None:
        print("ColPart",ColPart, type(ColPart))
#        probable_parts.append(x for x in ColPart.split(","))
#    if ColPart == None:
#        ColPart = []
#    else:
...
#    probable_parts.append(x for x in Referents.split(","))
#    if Referents == None:
#        Referents = []
#    else:
#        Referents = eval(Referents)
    print("probable_parts before Name", probable_parts)
...
#    probable_parts = ColPart + Referents
    ...
    print("probable_parts", probable_parts)
    if parts[DSUtag] != None:
        probable_parts.append(x for x in eval(parts[DSUtag]))
    All_parts = list(set(probable_parts))
    parts[DSUtag] = str(All_parts)
    print(parts[DSUtag])
