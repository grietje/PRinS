...
Now we have several functions that together gather information about participant references, and some functions to manage the workflow of all these references. The order of these functions is not intuitive, because functions used by other functions need to be declared before they can be used. A short overview might help:

\begin{tikzpicture}[outline/.style={draw=#1,thick,fill=#1!50},>=stealth',thick,black!50,text=black,every new ->/.style={shorten >=.3cm}]
\centering
\node [outline=blue] (basic) {Basic script};
\node [outline=blue] (flow) [below left = of basic] {Flow_clan};
\draw [shorten > = .1cm, ->] (basic.south) -- (flow.north);

\node [outline=blue] (prepare) [below = of flow] {* Ask_potential_ref};
\node [outline=blue] (recognize) [below = of prepare] {** Recognize};
\node [outline=blue] (clan) [below = of recognize] {Interactive_clan};
\node [outline=blue] (add) [below = of clan] {* Add_data};
\draw [shorten > = .1cm, ->] (recognize.south) -- (clan.north);

\node [outline=red] (pt) [below right = of add] {gcdata.pt};
\draw [shorten > = .1cm, ->] (add.south) -- (pt.north);

\node [outline=blue] (prepare2) {Ask_potential_ref};
\node [outline=blue] (recognize) [below = of prepare2] {* Recognize};
%\draw [shorten > = .1cm, ->] (flow.south) -- (clan.north);
%\node [outline=blue] (single) [below left = of ?????] {Interactive_single};
%\draw [shorten > = .1cm, ->] (flow.south) -- (single.north east);
%\draw [shorten > = .1cm, ->] (single.south east) -- (pt.north);

%\node [outline=blue] (functions) {Functions};
%\node [outline=blue] (recognize) {Recognize};
%\node [outline=blue] (add) {Add_data};


\end{tikzpicture}

\begin{tabbing}
Functions \= This script searches for the function and grammatical role that references have, as indicated in the ETCBC database.\\
...
Prepare / Ask_potential_ref \> Some basic information about a reference, that can be found automatically, is gathered before the start of the interactive procedure.\\
Add_data \> Automatically add data to entries that are going to be fed to gcdata.pt \\
Interactive_single \> This function asks interactively for input about suffixes that are not part of a larger `clan' (i.e. a multi-layered reference), but occur on their own. \\
Interactive_clan \> This function asks interactively for input about references that are related to each other, i.e. that form a multi-layered `clan'. Several proposals are shown, and can be accepted, rejected or adapted. \\
Flow_clan \> Overarching flow for clans to let the computer create as much data as possible about selected participant references and do a proposal to the user \\
...
""" If a participant has already been treated before, it is likely that some
    personal data has remained the same, and can therefore be re-used in the
    participant reference currently under consideration. This procedure of 
    re-using already entered information also helps guard consistency.
"""
def recognize(Des):
...
""" Some basic information about a reference, that can be found automatically,
    is gathered before the start of the interactive procedure.
"""
...
#    current = data[candidate]
    if node_type == "word+": # Reference is a word or higher-level reference
        ...
        if nodetype == "word":
            ...
            current["Nodetype"] = "4"
        if not get_cons_of_word_list(give_me_your_words(cand)) == "KL": # `KL' is not an independent reference, so the person it refers to is different every time. Therefore it is useless to run the function `Recognize' in this case.
...
#        cand = candidate[:-1]
    ...
    if current["Nodetype"] in ("1","2"): # Smaller than phrase, so can have PhANr
        ...
        current["PhANr"] = 00
    if current["Nodetype"] in ("1","2"): # Smaller than subphrase, so can have SPhNr
        current["SPhNr"] = L.u('subphrase',current["Node"])
    else:
        current["SPhNr"] = 00
    if current["Nodetype"] in ("1","2"): # Suffix or word, so WNr is applicable
    ...
    if current["Nodetype"] == "2": # Reference is a word
        ...
        if F.etcbc4_ft_function.v(L.u('phrase',current["Node"])) in ("ExsS","IntS","ModS","NCoS","PrcS","PreS"):
            current["Role"] = "Subject"
        elif F.etcbc4_ft_function.v(L.u('phrase',current["Node"])) in ("PreO","PtcO"):
            current["Role"] = "Object"
...
\begin{python}
""" This function asks interactively for input about suffixes that are not part
    of a larger `clan' (i.e. a multi-layered reference), but occur on their own.
"""
...
\begin{python}
""" This function asks interactively for input about references that are related
    to each other, i.e. that form a multi-layered `clan'. Several proposals are
    shown, and can be accepted, rejected or adapted.
"""
def interactive_clan(data,clan_members):
    format_string_a = "{0!s: <9} {1!s: <15} {2!s: <40}"
    format_string_b = "********* Nodetype: {0!s: <4}, Destype: {1!s: <4}, Name: {2!s: <10}, Anim: {3!s: <4}, Human: {4!s: <4}, Col: {5!s: <4}"
...
#    print("Col: 1=Individual, 2=Collective, 3=Individual in compound design, 4=Plural")
    for candidate in clan_members:
        ...
        print(format_string_b.format(current["Nodetype"], current["Destype"], current["Name"], current["Anim"], current["Human"], current["Col"]))
