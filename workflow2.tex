...
\begin{itemize}
    \item The \mi{minimal quotative frame} or \mi{quotative frame proper} that only contains the clause or clauses with the verb of speech. In the case of \hebr{L>MR}, the QF includes the clause immediately preceding \hebr{L>MR} because \hebr{L>MR} is not an independent verb on itself.
    \begin{itemize}
...
#    DSUdata["ClANrs"] = repr(ClANrs)
    ...
    Miller = input("Miller? Default No ") or "N"
    if Miller != "N":
        MillerQF1 = input("Clause_atoms that are MillerQF (type comma-separated numbers): ")
        MillerQF2 = ""
        for y in MillerQF1.split(","):
            MillerQF2 += str(potential_QF[int(y)]) + ","
            DSUdata["MillerQF"] = MillerQF2
        DSUdata["MillerSp"] = input("MillerSp: ") or ""
        DSUdata["MillerSptype"] = input("MillerSptype: ") or None
        DSUdata["MillerAd"] = input("MillerAd: ") or ""
        DSUdata["MillerAdtype"] = input("MillerAdtype: ") or None
    Long = input("Long? Default No ") or "N"
    if Long != "N":
        LongQF1 = input("Clause_atoms that are LongQF (type comma-separated numbers): ")
        LongQF2 = ""
        for y in LongQF1.split(","):
            LongQF2 += str(potential_QF[int(y)]) + ","
            DSUdata["LongQF"] = LongQF2
        DSUdata["LongSp"] = input("LongSp: ") or ""
        DSUdata["LongSptype"] = input("LongSptype: ") or None
        DSUdata["LongAd"] = input("LongAd: ") or ""
        DSUdata["LongAdtype"] = input("LongAdtype: ") or None
