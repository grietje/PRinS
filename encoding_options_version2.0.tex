...
for node in NN():
    otype = F.etcbc4_db_otype.v(node)
    if otype == "book" and F.etcbc4_sft_book.v(node) not in ("Genesis","Exodus"):
        break
    elif otype == "chapter":
        chapter = F.etcbc4_sft_chapter.v(node)
    elif otype == "verse":
        verse = F.etcbc4_sft_verse.v(node)
    elif otype == "word" and chapter == "13" and verse == "8":
...
#        DSU = (F.etcbc4_ft_txt.v(node).count("Q") > 0)    # If the text_type contains a Q, we consider the clause as Direct Speech
        ...
        if F.etcbc4_ft_lex.v(node) == ">T" and F.etcbc4_ft_prs.v(node) in ("absent","n/a"):
            check = True
        elif F.etcbc4_ft_sp.v(node) == "prps" and check == True:
            print(label, node)
            check = False
        else:
            check = False
