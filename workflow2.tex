    ...
    for candidate in clan_members:
        ...
        if copy == "Y":
            ...
            if copy2 == "Y":
                ...
                data[copy2_ref] = data[candidate]
        elif copy == "X":
            data.pop(candidate, None)
            clan_members2.remove(candidate)
            if clan_members2 == []:
                return
    clan_members3 = clan_members2
...
def major_flow(place, node, node_type):
    ...
    clan_members = []
    data = {}
    if not F.etcbc4_ft_g_cons.v(node) == F.etcbc4_ft_g_cons.v(parent[node]):
    ...
    for candidate in give_me_your_daughters(node):
        if F.etcbc4_db_otype.v(candidate) == "subphrase" and F.etcbc4_ft_rela.v(candidate) == "rec":
            ...
            clan[candidate] = "Attr "
        if F.etcbc4_ft_g_cons.v(candidate) == F.etcbc4_ft_g_cons.v(list(C.mother.v(candidate))[0]):
            clan[list(C.mother.v(candidate))[0]] = clan[candidate]
            candidate = list(C.mother.v(candidate))[0] 
